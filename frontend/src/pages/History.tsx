...
const History = () => {
  ...
  useEffect(() => {
    const loadScans = async () => {
      setLoading(true);
      if (user) {
        const dbScans = await fetchUserHistory(user.uid);
        if (dbScans.length > 0) {
          setScans(dbScans.map((s: any) => ({ ...s, vulnerabilities: s.vulnerabilities || [] })));
          setLoading(false);
          return;
        }
      }
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground font-body mt-1">
                ...
                {user && <span className="text-primary ml-1">• Synced</span>}
              </p>
            </div>
            {scans.length > 0 && (
              <Button
