  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userRef, (snapshot) => {
       if (snapshot.exists()) {
          ...
          setUsage(data.dailyScansUsed || 0);
          setUserPlan(data.plan || 'free');
          const planLimit = (data.plan === 'pro') ? 200 : (data.plan === 'enterprise' ? 999999 : 10);
          setUseLimit(planLimit);
       } else {
          // Initialize user profile
          const initialKey = 'sw_' + [...Array(24)].map(() => Math.random().toString(36)[2]).join('');
          setDoc(userRef, { 
            email: user.email, 
            apiKey: initialKey, 
            dailyScansUsed: 0, 
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-4 bg-slate-900 rounded-xl border border-white/5">
                       <div className="flex justify-between items-center mb-2">
                          <span className="text-xs font-medium text-slate-300 italic">{userPlan.charAt(0).toUpperCase() + userPlan.slice(1)} Tier</span>
                          <span className="text-xs font-bold">{usage}/{useLimit > 1000 ? '∞' : useLimit}</span>
                       </div>
                       <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-primary" style={{ width: `${Math.min(100, (usage/useLimit)*100)}%` }} />
                       </div>
                    </div>
                  </div>
              </div>
